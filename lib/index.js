...
AutoSequelize.prototype.run = function (callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function (table, _callback) {
      ...
      var tsTableDef = self.options.typescript ? 'interface ' + tableName + 'Model extends Model {' : '';
      ...
      _.each(fields, function (field, i) {
        ...
        _.each(fieldAttr, function (attr, x) {
          ...
          if (attr === "foreignKey") {
            ...
            if (self.tables[table][field][attr] === true && !_.has(self.tables[table][field], 'foreignKey'))
              text[table] += spaces + spaces + spaces + "autoIncrement: true";
            else return true
          }
          ...
          if (self.options.typescript && count === 0) {
            count++;
      ...
      text[table] += ") as " + tableName + "ModelStatic;\n};\n\n";
      text[table] += tsTableDef + '\n}\n';
      text[table] += tsHelper.model.getModelStatic(tableName);
      text[table] += "\nexport default " + tableName + "ModelInit;";
      if (self.options.typescript) {
        var fileName = (self.options.dashesForFileName) ? _.kebabCase(tableName) : tableName;
        typescriptFiles[0] += '\nexport { default as ' + tableName + 'ModelInit } from "./' + fileName + '";';
      }
      _callback(null);
    }, function () {
...
AutoSequelize.prototype.write = function (attributes, typescriptFiles, callback) {
  ...
  if (self.options.typescript) {
    if (typescriptFiles !== null && typescriptFiles.length >= 1) {
      fs.writeFileSync(path.join(self.options.directory, 'index.ts'), typescriptFiles[0], 'utf8');
    }
  }
  ...
  function createFile(table, _callback) {
    ...
    if (self.options.camelCaseForFileName) {
      fileName = _.camelCase(table);
    } else if (self.options.dashesForFileName) {
      fileName = _.kebabCase(table);
    }
